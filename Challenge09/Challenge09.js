...
const employeesBonus = (arr) => {
    for(let i=0; i < arr.length; i++){
        if ( arr[i].workHours > 8 ){
            arr[i].salary = parseInt(arr[i].salary.substring(0, arr[i].salary.length - 1)) + 100  + "$";
        }
        else {
            arr[i].salary = parseInt(arr[i].salary.substring(0, arr[i].salary.length - 1)) + 50  + "$";
        }
    }
    return arr;
}
...
const mostExpensive = (budget, mouseArray, keyBoardArray) => {
    ...
    let newMouseArray = mouseArray.sort();
    let newKeyBoardArray = keyBoardArray.sort();
    for (let i = 0; i < newMouseArray.length; i ++){
        for (let j = 0; j < newKeyBoardArray.length; j ++){
            if (newMouseArray[i] + newKeyBoardArray[j] <= budget)
                mostExpensiveValue = newMouseArray[i] + newKeyBoardArray[j];
            else continue;
        }
    }
    return mostExpensiveValue;
