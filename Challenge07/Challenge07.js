...
const applicationsStatics = (arr) => {
    ...
    for (let i = 0 ; i < arr.length ; ++i){
        ...
        switch(arr[i].tech){
            case "JS": javaScript_Devs++; break;
            case ".Net": dotNet_Devs++; break;
            case "Java": java_Devs++; break;
            case "Python": python_Devs++; break;
        }
    }
    return {
...
const classesAvg = (data) => {
    for (let x = 0; x <= data.grades.length - 1; ++x) {
        for (let y = 0; y <= data.grades[x].classes.length - 1; ++y) {
            let sum = 0;
            for ( let z = 0; z <= data.grades[x].classes[y].classScores.length - 1; ++z) {
                sum += data.grades[x].classes[y].classScores[z];
            }
            data.grades[x].classes[y].avg = Math.floor(sum / data.grades[x].classes[y].classScores.length);
        }
      }
      return data;
};
